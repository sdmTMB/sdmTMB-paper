...
## pak::pkg_install("sdmTMB/sdmTMB", dependencies = TRUE)
...
dat <- select(dogfish,
  lon = longitude, lat = latitude, year,
...
## ----dog-residuals, fig.cap="Simulation-based randomized quantile residuals from the \\pkg{DHARMa} package.", fig.asp=0.68, echo=TRUE, out.width="4in"----
set.seed(123)
...
if (!file.exists("ne_10m_lakes.shp")) {
  ...
  sf::st_write(lakes, "ne_10m_lakes.shp")
}
