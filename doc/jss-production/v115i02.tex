...
\Keywords{Gaussian Markov random field, GLMM, SPDE, species distribution modeling, \proglang{R} package, spatiotemporal, Template Model Builder}
...
One common application in the field of ecology is species distribution models (SDMs), hence the package name (i.e., SDMs with \pkg{TMB}), although the package is widely applicable to other fields and any geostatistical data collected continuously in space and discretely indexed in time.
...
The vector $\bm{\omega}$ is assumed drawn from a multivariate normal distribution with a covariance matrix $\bm{\Sigma}_\omega$,
\[
...
that no absolute values of log-likelihood gradients with respect to fixed effects are $\ge$ 0.001,
...
$$
%
where $n$ represents the number of left-out data points, $\log \mathcal{L}$ is a symbol for log likelihood, $y_i$ represents left-out data point $i$, and $\hat{y}_i$ represents the prediction for left-out data point $i$ with the fixed effects at their MLEs and random effects at their empirical Bayes estimates.
