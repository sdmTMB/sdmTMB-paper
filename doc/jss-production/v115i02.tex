...
\Keywords{Gaussian Markov random field, GLMM, SPDE, species distribution modeling, \proglang{R} package, spatiotemporal model, Template Model Builder}
...
Packages such as \pkg{VAST} \citep{thorson2019}, \pkg{tinyVAST} \citep{thorson2024tinyvast}, and \pkg{inlabru} \citep{bachl2019} are powerful user interfaces to fit spatial models that use the SPDE, but they lack a modular interface familiar to those who have used \pkg{lme4} or \pkg{glmmTMB}, lack some functionality, or may be challenging to learn for some users.
...
One common application in ecology is species distribution models (SDMs), hence the package name (i.e., SDMs with \pkg{TMB}), although the package is widely applicable to other fields and any geostatistical data collected continuously in space and discretely indexed in time.
...
A more interpretable derived parameter than the spatial decorrelation rate $\kappa$ is the spatial range --- the distance at which two points are nearly independent.
...
$\bm{h} = \bm{s}_j - \bm{s}_k$, with two estimated parameters that describe the direction
of anisotropy (the major axis) and the ratio of the major and minor axes
...
New observations can be simulated from a fitted model with \fct{simulate} or data can be simulated from scratch (without fitting a model) with \fct{simulate\_new}.
...
Conditional model:
                     coef.est coef.se
(Intercept)             -0.44    0.42
poly(log(depth), 2)1   -75.66    9.31
poly(log(depth), 2)2  -109.79    9.27

Matérn range: 41.23
Spatial SD: 1.70
ML criterion at convergence: 1034.493
...
# A tibble: 3 x 5
  ...
  <chr>                   <dbl>     <dbl>    <dbl>     <dbl>
1 (Intercept)            -0.436     0.424    -1.27     0.394
2 poly(log(depth), 2)1  -75.7       9.31    -93.9    -57.4  
...
# A tibble: 2 x 5
  term    estimate std.error conf.low conf.high
  <chr>      <dbl>     <dbl>    <dbl>     <dbl>
1 range      41.2      9.32     26.5      64.2 
2 sigma_O     1.70     0.233     1.30      2.22
\end{Soutput}
\end{Schunk}
...
\begin{Soutput}
        df      AIC delta_AIC
fit_dpl 19 11042.10   0.00000
fit_dpg 19 11110.56  68.45874
fit_dl  19 11131.32  89.21822
fit_dg  19 11207.94 165.83778
fit_tw  12 11355.75 313.65031
\end{Soutput}
\end{Schunk}
%
...
\begin{Soutput}
            df      AIC
fit_dpl_ar1 21 11037.47
fit_dpl     19 11042.10
fit_dpl_iso 17 11069.27
\end{Soutput}
\end{Schunk}
%
...
Conditional model:
            coef.est coef.se
(Intercept)     2.33    0.56
slog_depth      0.24    0.65

Smooth terms:
                 Std. Dev.
sd__s(log_depth)      3.18

Time-varying parameters:
                 coef.est coef.se
(Intercept)-2004     0.31    0.62
(Intercept)-2005     0.35    0.66
(Intercept)-2006     0.40    0.70
(Intercept)-2007     0.32    0.62
(Intercept)-2008     0.24    0.55
(Intercept)-2009     0.23    0.54
(Intercept)-2010     0.22    0.53
(Intercept)-2011     0.07    0.46
(Intercept)-2012    -0.07    0.48
(Intercept)-2013    -0.08    0.46
(Intercept)-2014    -0.09    0.44
(Intercept)-2015    -0.04    0.42
(Intercept)-2016     0.00    0.42
(Intercept)-2017    -0.02    0.44
(Intercept)-2018    -0.04    0.44
(Intercept)-2019    -0.14    0.47
(Intercept)-2020    -0.24    0.53
(Intercept)-2021    -0.34    0.62
(Intercept)-2022    -0.27    0.55
rho-(Intercept)      0.86    0.28

Spatiotemporal AR1 correlation (rho): 0.74
Matérn anisotropic range (spatial): 16.7 to 217.1 at 130 deg.
Spatial SD: 0.89
...
Conditional model:
            coef.est coef.se
(Intercept)     2.21    0.20
slog_depth      0.09    0.25

Smooth terms:
                 Std. Dev.
sd__s(log_depth)      0.64

Time-varying parameters:
                 coef.est coef.se
(Intercept)-2004     0.43    0.27
(Intercept)-2005     0.26    0.31
(Intercept)-2006     0.14    0.24
(Intercept)-2007     0.04    0.30
(Intercept)-2008    -0.05    0.26
(Intercept)-2009     0.20    0.30
(Intercept)-2010     0.50    0.27
(Intercept)-2011     0.31    0.30
...
(Intercept)-2018    -0.28    0.24
(Intercept)-2019    -0.27    0.35
(Intercept)-2020    -0.31    0.35
(Intercept)-2021    -0.42    0.24
(Intercept)-2022    -0.20    0.25
rho-(Intercept)      0.63    0.38

Dispersion parameter: 1.10
Spatiotemporal AR1 correlation (rho): 0.24
Matérn anisotropic range (spatial): 3.4 to 44.1 at 130 deg.
Spatial SD: 0.43
Spatiotemporal marginal AR1 SD: 0.90

ML criterion at convergence: 5497.734
